...
public class OrdersServlet extends HttpServlet {
    ...
    void getOrder(HttpServletRequest request, HttpServletResponse response, int memberNo) throws IOException {
        ...
        for (int i = 0; i < orders.size(); i++) {
            ...
            String status = ORDER_PAY_STATUS_MAPPING[orders.get(i).getOpaRealStatus()];
            if(orders.get(i).getOpaRealStatus() == 2)
                status += ", <a href=\"/BuyGo/front_end/pages/member/opa/second_checkout.html?id=" + orders.get(i).getOpaSoNo() + "\">前往結帳</a>";
            ordersArray[i].setOpaRealStatus(status);

...
//		HttpSession session = request.getSession(); // 取得當前請求的Session
		...
		Double memberCurrentWalletAmount = SERVICE.loadMemberWalletAmountByMemberNo(deductionWalletAmountDTO.getMemberNo()); // 查詢前端欲更改錢包金額的會員的目前錢包金額
		System.out.println("memberCurrentWalletAmount:" + memberCurrentWalletAmount);
		System.out.println("deductionWalletAmountDTO.getDeductionAmount():" + deductionWalletAmountDTO.getDeductionAmount());
		...
		// 修改錢包金額至member Table
		boolean modifyWalletAmountIsSuccess = SERVICE.modifyWalletAmountByMemberNoAndWalletAmount(deductionWalletAmountDTO.getMemberNo(), (memberCurrentWalletAmount - deductionWalletAmountDTO.getDeductionAmount()));
		
		if(addWalletTransHistIsSuccess && modifyWalletAmountIsSuccess) {
			deductionWalletAmountDTO.setSuccessful(true); // 若處理動作皆成功，傳送true至前端
			deductionWalletAmountDTO.setMessage("交易後的錢包餘額：" + (memberCurrentWalletAmount - deductionWalletAmountDTO.getDeductionAmount()));
		}
		else {
			deductionWalletAmountDTO.setSuccessful(false);
		}

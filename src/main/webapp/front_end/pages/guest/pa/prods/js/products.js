jQuery(function ($) {
	...
	*/
	...
	if ($product_layout.length) {
		...
		$product_layout.imagesLoaded(function () {
			if (!$product_layout.hasClass('isotope')) {
				$product_layout.isotope({
					itemSelector: '.mix-item'
				});
			} else {
				$product_layout.isotope('destroy');
				$product_layout.isotope({
					itemSelector: '.mix-item'
				});
			}
	...
	*/
	...
	$(".input-min").on("change", function(){
		$(".range-slider-value > .min").text("$" + $(".input-min").val());
		$(".noUi-origin-lower > input").val($(".input-min").val());
		let maxVal = $(".range-slider").attr("data-max");
		let leftPercentage = parseFloat(parseInt($(".input-min").val()) / parseInt(maxVal) * 100).toFixed(2)+"%";
		$(".noUi-origin-lower").css("left", leftPercentage);
		priceSlider([parseInt($(".input-min").val()), parseInt($(".input-max").val())]);
	});

	$(".input-max").on("change", function(){
		$(".range-slider-value > .max").text("$" + $(".input-max").val());
		$(".noUi-origin-upper > input").val($(".input-max").val());
		let maxVal = $(".range-slider").attr("data-max");
		let leftPercentage = parseFloat(parseInt($(".input-max").val()) / parseInt(maxVal) * 100).toFixed(2)+"%";
		$(".noUi-origin-upper").css("left", leftPercentage);
		priceSlider([parseInt($(".input-min").val()), parseInt($(".input-max").val())]);
	});
});
